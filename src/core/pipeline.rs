...
pub fn run_pipeline(ctx: &mut Context) -> Result<(), (ExitCode, String)> {
    ...
    info!("Bounding box: {:.1} x {:.1} x {:.1} mm", bbox.size().x, bbox.size().y, bbox.size().z);
    ...
    if size.x > 2000.0 || size.y > 2000.0 || size.z > 2000.0 {
        warn!("Model bounding box is very large: {:.1} x {:.1} x {:.1} mm", 
            size.x, size.y, size.z);
    }
    ...
    if ctx.stats.non_manifold_edges > ctx.stats.triangles_in / 10 {
        ...
        return Err((ExitCode::MeshUnrecoverable, 
            format!("Mesh repair failed — too many non-manifold edges (>10%)")));
    }
    
    info!("Mesh repaired ({} holes filled, {} normals fixed, {} non-manifold edges)", 
    ...
    let memory_limit = ctx.config.memory_limit.unwrap_or(ctx.available_memory * 75 / 100);
    let memory_limit_mb = memory_limit / (1024 * 1024);
    
    info!("Memory estimate: {} MB, available: {} MB", 
        memory_estimate / (1024 * 1024), memory_limit_mb);
    ...
    let decimate_ratio = if let Some(ratio) = ctx.config.decimate {
        ...
        warn!("Auto-decimating to {:.0}% to fit memory budget", ratio * 100.0);
    ...
    ctx.suggested_split_axis = orientation::analyze_orientation(
        ctx.original_mesh.as_ref().unwrap_or(ctx.mesh.as_ref().unwrap())
    );
    ...
    // Set wall thickness
    ctx.decisions.wall_thickness = ctx.config.wall_thickness
    ...
    // Set threads
    ctx.decisions.threads = ctx.config.threads
    ...
    let boolean_result = boolean::boolean_subtract(
        &mold_block,
    ...
    let cavity_mesh = match boolean_result {
        ...
        Err(e) => {
            ...
            return Err((ExitCode::BooleanFailed, 
    ...
    let (mold_a, mold_b) = split::split_mesh(&cavity_mesh, split_axis_vec, split_point);
    ...
    let base_stem = ctx.config.input.file_stem()
        .map(|s| s.to_string_lossy().to_string())
        .unwrap_or_else(|| "model".to_string());
    
    let output_dir = ctx.config.output_dir.clone()
        .unwrap_or_else(|| ctx.config.input.parent().map(|p| p.to_path_buf()).unwrap_or_default());
    
    // Export mold parts
    for (i, part) in mold_a.into_iter().chain(mold_b.into_iter()).enumerate() {
        let suffix = if i == 0 { "mold_A" } else { "mold_B" };
    ...
    info!("Done in {:.1}s", ctx.stats.processing_time_ms.unwrap_or(0) as f32 / 1000.0);
...
pub fn validate_mesh(path: &Path) -> Result<usize, String> {
    let mesh = loader::load_mesh(path, Unit::Millimeters)
